...
If offline or in isolated sandbox environments (when WebFetch cannot reach the remote URL), the agent falls back to the internal reference documentation located at `docs/explanation/web-interface-improvements.md`.

### 2. Execution Workflow for AI Agents

1. **Target Identification**: Specify files or patterns (e.g., `Web Ui/*.html`, `js/*.js`). If no files are specified, the agent will prompt the user for files/pattern or automatically identify modified or high-impact interface files.
