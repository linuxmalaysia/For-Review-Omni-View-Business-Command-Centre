...
> - Completely **free** on Render with unlimited static bandwidth allowance.
...
### 1. Error: `bash: line 1: uvicorn: command not found` (Exit Status 127)
...
- **Symptom:** Deployment log fails during the start/deploy phase with:
  ...
  ```
- **Root Cause:** The Render service was configured as a **Web Service** (Python runtime) using a default or copied start command (`uvicorn ...`), rather than a **Static Site**. Omni-View is a static web application and documentation portal that does not require an ASGI web server like Uvicorn.
- **Resolution:**
  1. In the Render Dashboard, check the service type. If it is set to a Web Service expecting a start command, change the service or delete it and create a new **Static Site** service.
