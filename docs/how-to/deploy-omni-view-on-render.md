...
- **Resolution:**
  1. In the Render Dashboard, check the service type. If it is set to a Web Service expecting a start command, change the service or delete it and create a new **Static Site** service (note that an existing Web Service cannot be converted directly into a Static Site in-place).
